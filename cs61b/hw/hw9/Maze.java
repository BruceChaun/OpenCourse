...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
	...
	int hWallSize = horiz * (vert - 1);
	int vWallSize = (horiz - 1) * vert;
	int wallSize = hWallSize + vWallSize;

	DisjointSets ds = new DisjointSets(horiz * vert);
	int[][] randomWalls = new int[wallSize][3];

	// put internal walls into array
	for (i = 0; i < horiz; i++) {
		for (j = 0; j < vert - 1; j++) {
			int n = j * horiz + i;
			randomWalls[n][0] = 0;		// 0 means horizontal walls
			randomWalls[n][1] = i;		// x
			randomWalls[n][2] = j;		// y
		}
	}
	for (i = 0; i < horiz - 1; i++) {
		for (j = 0; j < vert; j++) {
			int n = j * (horiz-1) + i + hWallSize;
			randomWalls[n][0] = 1;		// 1 means vertical walls
			randomWalls[n][1] = i;		// x
			randomWalls[n][2] = j;		// y
		}
	}
	...
	// reorder wall arrays
	for (i = wallSize - 1; i > 0; i--) {
		int r = randInt(i + 1);
		int[] tmp = new int[3];
		tmp[0] = randomWalls[r][0];
		tmp[1] = randomWalls[r][1];
		tmp[2] = randomWalls[r][2];
		randomWalls[r][0] = randomWalls[i][0];
		randomWalls[r][1] = randomWalls[i][1];
		randomWalls[r][2] = randomWalls[i][2];
		randomWalls[i][0] = tmp[0];
		randomWalls[i][1] = tmp[1];
		randomWalls[i][2] = tmp[2];
	}

	// use disjoint set to eliminate walls
	int wallNum = 0;
	while (wallNum < wallSize) {
		int direction = randomWalls[wallNum][0];
		int x = randomWalls[wallNum][1];
		int y = randomWalls[wallNum][2];
		int num1 = y * horiz + x;
		int num2 = num1 + 1;
		if (direction == 0)
			num2 = num1 + horiz;
