...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
	...
	while (cur != null) {
		int cmp = key.compareTo((Comparable) cur.entry.key);
		if (cmp == 0) {
			return cur;
		} else if (cmp < 0) {
			cur = findHelper(key, cur.leftChild);
		} else {
			cur = findHelper(key, cur.rightChild);
		}
	}
    return null;
  }
		...
		if (node.rightChild == null) {
			...
			if (cur.rightChild == null) {
				...
				if (cur.parent != node) {
					...
					cur.rightChild.parent = node;
					node.rightChild = cur.rightChild;
				}
			}
		}
	}
