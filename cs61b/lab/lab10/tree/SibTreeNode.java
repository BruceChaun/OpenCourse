...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
	...
	int childrenNum = children();
	if (c <= 1) {
		newChild.nextSibling = firstChild;
		firstChild = newChild;
	} else if (c > childrenNum) {
		SibTreeNode kid = (SibTreeNode) child(childrenNum);
		kid.nextSibling = newChild;
	} else {
		SibTreeNode kid = (SibTreeNode) child(c - 1);
		newChild.nextSibling = kid.nextSibling;
		kid.nextSibling = newChild;
	}
	myTree.size++;
  }
  ...
  public void removeLeaf() throws InvalidNodeException {
	...
	if (childrenNum == 0) {
		if (parent == null) {
			...
			SibTreeNode n = parent.firstChild;
